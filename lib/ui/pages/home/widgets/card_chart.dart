...
class _CardChartState extends State<CardChart> {
  ...
  Widget build(BuildContext context) {
    ...
    return ElevatedButton(
      onPressed: () => print(conta.toStringAsFixed(0)),
      child: const Text(Strings.HOME_INDICE),
    );
  }

  double totalCategory() {
    double value = 0;
    var filtred = widget.transactionOutcome
        .where((element) => element.category!.name == Strings.HOME_FOOD);
